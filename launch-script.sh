...
ExecStart=/bin/bash -c 'cd /opt/bitnami/projects && curl -o install-script.sh https://raw.githubusercontent.com/rjourdan/Starting-to-Code-on-AWS/step1-monolith-Lightsail/install-script.sh && chmod +x install-script.sh && ./install-script.sh && touch /var/lib/remarket-setup-complete'
RemainAfterExit=yes
